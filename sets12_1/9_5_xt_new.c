...
struct Friend *ReadInput();


int main() {
    struct Friend *p = NULL;
...
struct Friend *ReadInput() {
    int N;
    struct Friend *pHead = (struct Friend *) malloc(sizeof(struct Friend));
    pHead->Next = NULL;
    struct Friend *pTail = pHead;

    scanf("%d", &N);
    while (N > 0) {
        struct Friend *pNode = (struct Friend *) malloc(sizeof(struct Friend));
        pNode->Next = NULL;
        scanf("%s %d %s", pNode->name, &pNode->birthday, pNode->number);
...
//            todo 插在中间
            while (pTail && pNode->birthday > pTail->birthday) {
                pTail = pTail->Next;
            }
            pNode->Next =
